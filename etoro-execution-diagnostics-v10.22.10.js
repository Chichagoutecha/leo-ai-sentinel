...
function safeRequestBody(body) {
  ...
  try {
    ...
    const allowed = ['InstrumentId','Amount','Leverage','IsBuy','UnitsToDeduct','PositionId','OrderId','StopLossRate','TakeProfitRate','TrailingStopLoss'];
    const output = {};
    for (const key of allowed) if (Object.prototype.hasOwnProperty.call(parsed, key)) output[key] = parsed[key];
    return output;
  } catch {
...
function redactText(value) {
  ...
  const secrets = [process.env.ETORO_API_KEY, process.env.ETORO_USER_KEY, process.env.BOT_SECRET, process.env.OPENAI_API_KEY, process.env.UPSTASH_REDIS_REST_TOKEN, process.env.TWELVE_DATA_API_KEY].filter(Boolean);
...
function safeResponseHeaders(response) {
  const allowed = ['content-type','content-length','date','server','x-request-id','x-correlation-id','correlation-id','traceparent','request-id','retry-after','location'];
  const output = {};
  for (const name of allowed) {
    try { const value = response.headers.get(name); if (value != null) output[name] = value; } catch {}
  }
  return output;
}
...
function classifyExecutionResponse(response, rawText) {
  ...
  if (!is2xx) return { classification:'HTTP_ERROR', ambiguous:false, businessAcknowledged:false, businessRejected:true, parsedJson:parsed.parsed, jsonParseError:parsed.error };
  if (!trimmed) return { classification:'HTTP_2XX_EMPTY_BODY', ambiguous:true, businessAcknowledged:false, businessRejected:false, parsedJson:false, jsonParseError:null };
  if (parsed.parsed && isEmptyJson(parsed.value)) return { classification:'HTTP_2XX_EMPTY_JSON', ambiguous:true, businessAcknowledged:false, businessRejected:false, parsedJson:true, jsonParseError:null };
  if (parsed.parsed && parsed.value && typeof parsed.value === 'object') {
    ...
    if (success === false || errorCode != null) return { classification:'BUSINESS_REJECTED', ambiguous:false, businessAcknowledged:false, businessRejected:true, parsedJson:true, jsonParseError:null, businessFields:fields };
    if (orderId != null || positionId != null || statusId != null || success === true) return { classification:'BUSINESS_ACKNOWLEDGED', ambiguous:false, businessAcknowledged:true, businessRejected:false, parsedJson:true, jsonParseError:null, businessFields:fields };
  }
  return { classification:'HTTP_2XX_UNRECOGNIZED_BODY', ambiguous:true, businessAcknowledged:false, businessRejected:false, parsedJson:parsed.parsed, jsonParseError:parsed.error };
}
...
global.fetch = async function leoEtoroDiagnosticFetch(input, init = {}) {
  ...
  if (openOrderRequest && breakerDurationMs > 0 && Date.now() < breakerUntilMs) return makeBreakerResponse(url, method, requestId);

  const startedAtMs = Date.now();
  let response;
  try { response = await originalFetch(input, init); }
  catch (error) {
    if (executionRequest) publishDiagnostic({ diagnosticVersion:DIAGNOSTIC_VERSION, event:'ETORO_EXECUTION_HTTP_RESPONSE', at:nowIso(), classification:'NETWORK_ERROR', method, url:normalizeUrl(url), executionIntent:intent, breakerEligible:openOrderRequest, requestId:requestId || null, requestBody:safeRequestBody(init.body), durationMs:Date.now() - startedAtMs, errorName:error?.name || null, errorMessage:redactText(error?.message || String(error)) }, 'error');
    throw error;
  }
  ...
  const diagnostic = {
    ...
    responseBodyPreview:redactText(rawBody), parsedJson:classification.parsedJson, jsonParseError:classification.jsonParseError || null,
    bodyReadError, breakerArmed:false, breakerUntil:null
  };
