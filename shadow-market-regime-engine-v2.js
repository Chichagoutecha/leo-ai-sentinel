...
const VERSION = 'v10.24.2-market-regime-engine-2';
...
function softmax(scores){ const vals=REGIMES.map(r=>num(scores[r],0)); const m=Math.max(...vals); const ex=vals.map(v=>Math.exp((v-m)/18)); const s=ex.reduce((a,b)=>a+b,0)||1; return Object.fromEntries(REGIMES.map((r,i)=>[r,Math.round(ex[i]/s*10000)/10000])); }

function buildRawScores(input={}){
  const macro=input.macro||{}; const market=input.market||{}; const event=input.eventRisk||{};
...
function evaluateRegime(input={}, options={}){
  ...
  const previous=String(options.previousRegime||state.last?.regime||'NEUTRAL');
  const prevP=probs[previous]||0;
  const hysteresis=clamp(options.hysteresisMargin??0.08,0,0.3);
  ...
  state.evaluations+=1;
  const result={ version:VERSION, at:new Date(options.now||Date.now()).toISOString(), regime:stableRegime, confidence, probabilities:probs, rawScores:raw, candidateRegime:candidate, previousRegime:previous, hysteresisMargin:hysteresis, riskMultiplier,
...
module.exports={VERSION,REGIMES,buildRawScores,softmax,evaluateRegime,getState};
