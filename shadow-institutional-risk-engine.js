...
/** LEO-AI SENTINEL — Stage 15 Institutional Risk Engine 1.1 (Shadow only). */
const VERSION='v10.24.4.1-institutional-risk-engine-data-complete';
...
function portfolioSeries(weights,returns){const syms=Object.keys(weights).filter(s=>weights[s]>0);if(!syms.length)return[];const lengths=syms.map(s=>Array.isArray(returns[s])?returns[s].length:0);if(lengths.some(n=>n<=0))return[];const n=Math.min(...lengths);if(!Number.isFinite(n)||n<=0)return[];const out=[];for(let i=0;i<n;i++){let r=0;for(const s of syms){const series=returns[s]||[];r+=weights[s]*num(series[series.length-n+i]);}out.push(r);}return out;}
...
function positiveSymbols(weights){return Object.entries(weights).filter(([,w])=>w>0).map(([s])=>s);}
function historyCoverage(weights,returns,minObs){return positiveSymbols(weights).map(symbol=>({symbol,observations:Array.isArray(returns[symbol])?returns[symbol].filter(v=>Number.isFinite(Number(v))).length:0})).filter(x=>x.observations<minObs);}
function stressCoverage(weights,scenarios){const syms=positiveSymbols(weights);const missing=[];for(const [scenario,moves] of Object.entries(scenarios||{})){const absent=syms.filter(s=>!moves||!Object.prototype.hasOwnProperty.call(moves,s)||!Number.isFinite(Number(moves[s])));if(absent.length)missing.push({scenario,missingSymbols:absent});}return missing;}
...
function assessInstitutionalRisk(input={},options={}){
  const weights=normalizeWeights(input.weights||{});const returns=input.returns||{};
  ...
  const scenarios=input.stressScenarios||{};
  const missingStress=stressCoverage(weights,scenarios);
  if(Object.keys(scenarios).length&&missingStress.length)return inconclusive('INCOMPLETE_STRESS_COVERAGE',{observations:0,incompleteStressScenarios:missingStress,thresholds:{minObservations:minObs}});
  const series=portfolioSeries(weights,returns);
  if(series.length<minObs)return inconclusive('INSUFFICIENT_HISTORY',{observations:series.length,thresholds:{minObservations:minObs}});
...
module.exports={VERSION,mean,variance,quantile,maxDrawdown,portfolioSeries,stressLoss,riskContributions,normalizeWeights,historyCoverage,stressCoverage,assessInstitutionalRisk,getState};